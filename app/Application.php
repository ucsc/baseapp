...
{
    /**
     * Current Baseapp version. Must start with a v, and is synced with git tags/releases.
     *
     ...
     * If this installation of Baseapp has a CDN_URL configured, use it as the base.
    ...
    {
        $cdnUrl = trim(config('baseapp.cdn.url'), '/ ');

        return $cdnUrl ? $cdnUrl.'/'.trim(ltrim($name, '/')) : trim(asset($name));
    }

    /**
     * Get the latest version number of Baseapp from GitHub.
     *
     * @param Client $client
     *
     * @return string
    ...
    {
        return Cache::remember('latestBaseappVersion', 1 * 24 * 60, function () use ($client) {
            $client = $client ?: new Client();

            try {
                $v = json_decode(
                    $client->get('https://api.github.com/repos/ucsc/baseapp/tags')
                        ->getBody()
                )[0]->name;
